...
- **デプロイ**：1サイト + 1ターゲットで、ファイル同期だけ実行し、リモートのディレクトリがどう変わるか確認する。

以上で、wp-tether を「手を動かして再現・変更できる」ための最小限の道筋がそろっています。不明な章があれば、その章の該当ファイル名と「どこまで理解したか」をメモしておくと、あとから調べたり質問したりしやすくなります。
